...
describe("Cliente", () => {
   ...
   test("Puede comprar un paquete nuevo cuando el anterior está agotado", () => {
      ...
      cliente.comprarPaquete(paqueteChico(), FECHA_DE_COMPRA);
      cliente.registrarConsumo(new ConsumoInternet(1024, "Chrome", new Date(2024, 0, 2, 10, 0), new Date(2024, 0, 2, 11, 0)));
   ...
   test("Registrar un consumo de internet descuenta datos del paquete actual", () => {
      ...
      cliente.registrarConsumo(new ConsumoInternet(500, "Chrome", new Date(2024, 0, 2, 10, 0), new Date(2024, 0, 2, 11, 0)));
   ...
   test("No se puede registrar un consumo con el paquete vencido", () => {
      ...
      const consumo = new ConsumoInternet(500, "Chrome", new Date(2024, 0, 20, 10, 0), new Date(2024, 0, 20, 11, 0));
   ...
   test("Un consumo mayor a lo disponible falla y no queda en el historial", () => {
      ...
      const consumo = new ConsumoInternet(2000, "Chrome", new Date(2024, 0, 2, 10, 0), new Date(2024, 0, 2, 11, 0));
   ...
   test("El historial de consumos está ordenado por fecha aunque lleguen desordenados", () => {
      ...
      const consumoDelDia5 = new ConsumoInternet(500, "Chrome", new Date(2024, 0, 5, 10, 0), new Date(2024, 0, 5, 11, 0));
   ...
   test("El historial se puede filtrar por un rango de fecha y hora", () => {
      ...
      const consumoDelDia2 = new ConsumoInternet(500, "Chrome", new Date(2024, 0, 2, 10, 0), new Date(2024, 0, 2, 11, 0));
      const consumoDelDia10 = new ConsumoLlamada(30, new Date(2024, 0, 10, 10, 0), new Date(2024, 0, 10, 10, 30));
      const consumoDelDia20 = new ConsumoInternet(200, "Chrome", new Date(2024, 0, 20, 10, 0), new Date(2024, 0, 20, 11, 0));
   ...
   test("Con renovación automática, al consumir con el paquete vencido se recompra el mismo paquete", () => {
      ...
      cliente.registrarConsumo(new ConsumoInternet(500, "Chrome", new Date(2024, 1, 15, 10, 0), new Date(2024, 1, 15, 11, 0)));
   ...
   test("Con renovación automática, al consumir con el paquete agotado se recompra el mismo paquete", () => {
      ...
      cliente.registrarConsumo(new ConsumoInternet(100, "Chrome", new Date(2024, 0, 2, 15, 0), new Date(2024, 0, 2, 16, 0)));
   ...
   test("Con renovación automática pero sin saldo suficiente, el consumo falla por no tener paquete vigente", () => {
      ...
      const consumo = new ConsumoInternet(500, "Chrome", new Date(2024, 1, 15, 10, 0), new Date(2024, 1, 15, 11, 0));
   ...
   test("Un consumo de la app ilimitada del paquete no descuenta datos pero queda en el historial", () => {
      ...
      const paquete = new ConstructorDePaquetes()
         ...
         .construir();
      cliente.comprarPaquete(paquete, FECHA_DE_COMPRA);

      cliente.registrarConsumo(new ConsumoInternet(500, "WhatsApp", new Date(2024, 0, 2, 10, 0), new Date(2024, 0, 2, 11, 0)));
      cliente.registrarConsumo(new ConsumoInternet(500, "Chrome", new Date(2024, 0, 2, 12, 0), new Date(2024, 0, 2, 13, 0)));

      expect (cliente.obtenerDatosDisponibles()).toBe(2060);
      expect (cliente.historialDeConsumos().length).toBe(2);
   })
});
